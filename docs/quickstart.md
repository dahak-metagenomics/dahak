...
!!! warning "Examples in the Repository"
    ...
    Also see [`workflows/config/example_datafiles.json`](https://github.com/dahak-metagenomics/dahak/blob/snakemake/comparison/workflows/config/example_datafiles.json)
...
Each workflow has different build rules that require different information.
These should be specified in the workflow configuration JSON file.

See the [Workflow Configuration](config.md) page for an overview of how to
configure workflows with Snakemake.

Each workflow component also has a "Snakemake" page that covers all options
that can be set in the Snakemake workflow configuration and workflow parameter
JSON files. Use the navigation menu on the left-hand side to select the
workflow component of choice and select the "Snakemake" page.
...
This particular workflow component is covered on the [Read Filtering 
Snakemake](readfilt_snakemake.md) page.
