...
mkdir ~/data/qc
mkdir ~/data/qc/before_trim2

for i in *.fq.gz
do
        docker run -v /home/ubuntu/data:/data -it biocontainers/fastqc fastqc ${i} -o /data/qc/before_trim2
done
...
mkdir ~/data/qc/after_trim2

for i in *.trim2.fq.gz
do
        docker run -v /home/ubuntu/data:/data -it biocontainers/fastqc fastqc ${i} -o /data/qc/after_trim2
done
...
do
  ...
  docker run -v /home/ubuntu/data:/data -it quay.io/biocontainers/khmer:2.1--py35_0 interleave-reads.py \
        /data/${base}_1.trim2.fq.gz /data/${base}_2.trim2.fq.gz --no-reformat -o /data/$output --gzip

done

for i in ~/data/qc/*_trim2
do
